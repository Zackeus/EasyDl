#!/usr/bin/env python 
# -*- coding: utf-8 -*- 
# @Title : 
# @Author : Zackeus
# @File : flow_type.py 
# @Software: PyCharm
# @Time : 2019/4/24 15:19


from marshmallow import fields

from extensions import db, cache
from models.basic import BasicModel, BaseSchema
from utils import validates


class FlowTypeModel(BasicModel):
    """
    流程类型
    """
    __tablename__ = 'FLOW_TYPE'

    flow_type_name = db.Column(
        db.String(length=10),
        name='FLOW_TYPE_NAME',
        index=True,
        unique=True,
        nullable=False,
        comment='流程类型'
    )

    flow_dir = db.Column(
        db.String(length=20),
        name='FLOW_DIR',
        index=True,
        unique=True,
        nullable=False,
        comment='流程目录名'
    )

    loan_files = db.relationship(
        argument='LoanFileModel',
        back_populates='flow_type',
        cascade='all'
    )

    def dao_add(self, subtransactions=False, nested=False):
        super().dao_create()
        with db.auto_commit_db(subtransactions=subtransactions, nested=nested) as s:
            s.add(self)

    @cache.memoize()
    def dao_get_by_type(self, type_name):
        """
        根据类型字符查询
        :param type_name:
        :return:
        """
        return self.query.filter_by(flow_type_name=type_name).first()


class FlowTypeSchema(BaseSchema):
    """
    流程类型校验器
    """
    __model__ = FlowTypeModel

    flow_type_name = fields.Str(
        required=True,
        validate=validates.MyLength(min=1, max=10, not_empty=False),
        load_from='flowTypeName'
    )

    flow_dir = fields.Str(
        required=True,
        validate=validates.MyLength(min=1, max=20, not_empty=False),
        load_from='flowDir'
    )

    def only_create(self):
        return super().only_create() + ('flow_type_name', 'flow_dir')