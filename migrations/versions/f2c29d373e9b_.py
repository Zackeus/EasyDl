...
Revision ID: f2c29d373e9b
Revises: 
Create Date: 2019-06-07 15:40:24.065446
...
def upgrade():
    ...
    sa.Column('MD5_ID', sa.String(length=64), nullable=False, comment='文件MD5值'),
    sa.Column('FILE_NAME', sa.String(length=64), nullable=False, comment='文件名'),
    sa.Column('FILE_FORMAT', sa.String(length=20), nullable=False, comment='文件格式'),
    sa.Column('FILE_SIZE', sa.Numeric(precision=10, scale=2, asdecimal=False), nullable=False, comment='文件大小'),
    sa.Column('FILE_PATH_HASH', sa.String(), nullable=False, comment='文件路径密文'),
    sa.PrimaryKeyConstraint('ID')
    )
    op.create_index(op.f('ix_FILE_MD5_ID'), 'FILE', ['MD5_ID'], unique=False)
    ...
    op.create_index('ix_IMG_DATA_APP_ID_APP_SYS_ID', 'IMG_DATA', ['APP_ID', 'APP_SYS_ID'], unique=True)
...
def downgrade():
    ...
    op.drop_index('ix_IMG_DATA_APP_ID_APP_SYS_ID', table_name='IMG_DATA')
    op.drop_table('IMG_DATA')
    op.drop_index(op.f('ix_FILE_MD5_ID'), table_name='FILE')
    op.drop_table('FILE')
    # ### end Alembic commands ###
